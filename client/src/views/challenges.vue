	...
	<div>
		<div class="row">
			...
			<div class="col-6">
				...
				<div class="card mb-4" v-for="scenario of scenarios">
					<div class="card-header d-flex align-items-center" v-bind:class="[scenario.output.equal ? 'text-success' : 'text-danger']">
						...
						<h4 class="mb-0">{{scenario.name}}</h4>
					</div>
					<div class="card-body" v-if="!scenario.output.equal">
						...
						<div class="row mb-2" v-if="scenario.output.calculated.alert.message !== null">
							<div class="col-12">
								<alert v-bind:alert="scenario.output.calculated.alert"/>
							</div>
						</div>
						<div class="card">
							<div class="card-body">
								<json-builder v-bind:template="scenario.output.calculated.value" v-bind:convert="true" v-bind:fixed="true" v-bind:fixed-values="true"/>
							</div>
						</div>
					</div>
				</div>
		...
		methods: {
			...
			async getRawResult() {
				...
				for (const scenario of this.scenarios) {
					...
					scenario.output.calculated.value = result.outputs.main.value;
					...
					scenario.output.equal = JSON.stringify(scenario.output.expected) === JSON.stringify(scenario.output.calculated.value);
					if (scenario.output.equal) {
						this.progress += 1 / this.scenarios.length;
					}
				}
			},
			...
			displayAlert(scenario, message, state) {
				scenario.output.calculated.alert = {
					message: message,
					state: state
				}
			},
