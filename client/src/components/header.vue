<template>
	<nav class="navbar navbar-expand mb-4 p-0">
		...
		<div class="nav-link" style="display: flex; align-items: center" v-if="$route.params.workspace !== undefined" v-bind:style="{opacity: $store.state.listeners === 0 ? 0 : 1}">
			<span class="mr-1">{{$store.state.listeners}}</span>
		...
		<router-link class="nav-link" v-if="$route.params.workspace !== undefined" v-bind:to="'/' + $route.params.workspace + '/settings'">
			<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
