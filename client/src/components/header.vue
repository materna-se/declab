<template>
	<nav class="navbar navbar-expand mb-3 p-0">
		...
		<router-link class="nav-link" v-if="$route.params.workspace == undefined" v-bind:to="'/login'" aria-label="Login">
			{{ $store.state.user && $store.state.user.username != "demo" ? $store.state.user.username : "login" }}
			<svg class="svg-icon" width="24" height="24" viewBox="0 0 20 20">
