<template>
	<ul class="list-group">
		<li class="list-group-item" v-for="entry of entries">
			<h5 class="mb-3" style="display: flex; align-items: center">
				<small class="badge bg-primary me-2" style="line-height: inherit;font-size: 0.85rem;">{{entry.accessType}}</small>
			...
			<json-builder v-if="entryContextVisible" class="mt-3 mb-3" v-bind:template="entry.entryContext" v-bind:convert="true" v-bind:fixed="true" v-bind:fixed-values="true"/>

			<access-log class="mt-3 mb-3" v-if="entry.children.length > 0" v-bind:entries="entry.children"></access-log>

			<h6 class="d-flex align-items-center mb-0" v-on:click="exitContextVisible = !exitContextVisible">
				Exit Context
