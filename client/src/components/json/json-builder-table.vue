<template>
	<div>
		...
		<div class="input-group" v-else-if="value.type === 'number'" v-bind:style="{'opacity': value.value === value.template ? '0.6' : '1'}">
			...
			<input type="number" step="any" placeholder="Enter Value..." class="form-control" v-bind:value="value.value" v-bind:disabled="fixedValues" v-on:input="$set(value, 'value', $event.target.value === '' ? undefined : Number($event.target.value))">
		</div>
		<div class="btn-group" v-else-if="value.type === 'boolean'" v-bind:style="{'opacity': value.value === value.template ? '0.6' : '1'}">
			<button type="button" class="btn" v-bind:class="[value.value === undefined ? 'btn-light' : 'btn-white']" v-bind:disabled="fixedValues" v-on:click="$set(value, 'value', undefined)">
			...
			<button type="button" class="btn" v-bind:class="[value.value === true ? 'btn-light' : 'btn-white']" v-bind:disabled="fixedValues" v-on:click="$set(value, 'value', true)">
				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			...
			<button type="button" class="btn" v-bind:class="[value.value === false ? 'btn-light' : 'btn-white']" v-bind:disabled="fixedValues" v-on:click="$set(value, 'value', false)">
				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
		...
		<div class="input-group mt-compact" v-if="['object', 'array'].includes(value.type) && !fixed">
			...
			<div v-bind:class="[value.type === 'object' ? 'input-group-append': 'btn-group ml-auto']">
				<button type="button" class="btn btn-white" v-on:click="addValue('string', value)">
				...
				<button type="button" class="btn btn-white" v-on:click="addValue('dateTime', value)">
					<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block">
						<path d="M9 10v2H7v-2h2m4 0v2h-2v-2h2m4 0v2h-2v-2h2m2-7a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h1V1h2v2h8V1h2v2h1m0 16V8H5v11h14M9 14v2H7v-2h2m4 0v2h-2v-2h2m4 0v2h-2v-2h2z" fill="currentColor"></path>
					</svg>
				</button>
				<button type="button" class="btn btn-white" v-on:click="addValue('number', value)">
					<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block">
				...
				<button type="button" class="btn btn-white" v-on:click="addValue('boolean', value)">
					<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block">
						<path d="M19 19H5V5h10V3H5c-1.11 0-2 .89-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-8h-2m-11.09-.92L6.5 11.5 11 16 21 6l-1.41-1.42L11 13.17l-3.09-3.09z" fill="currentColor"></path>
					</svg>
				</button>
				<button type="button" class="btn btn-white" v-on:click="addValue('null', value)">
				...
				<button type="button" class="btn btn-white" v-on:click="addValue('object', value)">
					<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block">
				...
				<button type="button" class="btn btn-white" v-on:click="addValue('array', value)">
					<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block">
						<path d="M7 13v-2h14v2H7m0 6v-2h14v2H7M7 7V5h14v2H7M3 8V5H2V4h2v4H3m-1 9v-1h3v4H2v-1h2v-.5H3v-1h1V17H2m2.25-7a.75.75 0 0 1 .75.75c0 .2-.08.39-.21.52L3.12 13H5v1H2v-.92L4 11H2v-1h2.25z" fill="currentColor"></path>
					</svg>
				</button>
