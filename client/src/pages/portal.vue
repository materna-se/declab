<template>
	<div class="row">
		...
		<div class="col-12 col-sm-4 mb-4">
			...
			<div class="card mb-4" v-if="this.sessionToken === null">
				<div class="card-body">
				...
				<template v-if="Object.keys(laboratories).length !== 0">
					<div class="list-group-item list-group-item-action c-pointer d-flex align-items-center" v-for="(laboratory, uuid) in laboratories" :key="uuid" @click="enterLaboratory(uuid)" @keypress="simulateClick" tabindex="0">
						...
						<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block float-start">
							<path d="M14.06 9l.94.94L5.92 19H5v-.92L14.06 9m3.6-6c-.25 0-.51.1-.7.29l-1.83 1.83 3.75 3.75 1.83-1.83c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.2-.2-.45-.29-.71-.29m-3.6 3.19L3 17.25V21h3.75L17.81 9.94l-3.75-3.75z" fill="currentColor" v-if="laboratory.access === 'PUBLIC'"/>
		...
		methods: {
			...
			async getLaboratories() {
				this.laboratories = await Network.getLaboratoriesFromDb({
					laboratoryUuid: this.laboratoryUuid,
					query: this.query,
			...
			async loginUser() {
				...
				this.sessionToken = await Network.login({
					username: username,
					password: password
				});
