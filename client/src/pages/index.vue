<template>
	<div class="row">
		...
		<div class="col-12 col-sm-4 mb-4">
			<h5 class="mb-2">Available Laboratories</h5>
			<div class="input-group mb-2" v-if="Object.keys(laboratories).length !== 0">
				...
				<input placeholder="Search laboratories..." class="form-control" v-model="query" @keyup="getWorkspaces">
			</div>

			<div class="list-group">
				<template v-if="Object.keys(laboratories).length !== 0">
					<div class="list-group-item list-group-item-action c-pointer d-flex align-items-center" v-for="(laboratory, uuid) in laboratories" :key="uuid" @click="enterLaboratory(uuid)" @keypress="simulateClick" tabindex="0">
						<div class="me-auto">
							<p class="mb-0"><b>{{laboratory.name}}</b></p>
						...
						<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block float-start">
							<path d="M14.06 9l.94.94L5.92 19H5v-.92L14.06 9m3.6-6c-.25 0-.51.1-.7.29l-1.83 1.83 3.75 3.75 1.83-1.83c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.2-.2-.45-.29-.71-.29m-3.6 3.19L3 17.25V21h3.75L17.81 9.94l-3.75-3.75z" fill="currentColor" v-if="laboratory.visability === 0"/>
			...
			<div class="list-group">
				<template v-if="Object.keys(workspaces).length !== 0">
					<div class="list-group-item list-group-item-action c-pointer d-flex align-items-center" v-for="(workspace, uuid) in workspaces" :key="uuid" @click="enterWorkspace(uuid)" @keypress="simulateClick" tabindex="1">
						<div class="me-auto">
							<p class="mb-0"><b>{{workspace.name}}</b></p>
							<p class="mb-0">{{workspace.description}}</p>
						</div>
						<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block float-start">
							<path d="M14.06 9l.94.94L5.92 19H5v-.92L14.06 9m3.6-6c-.25 0-.51.1-.7.29l-1.83 1.83 3.75 3.75 1.83-1.83c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.2-.2-.45-.29-.71-.29m-3.6 3.19L3 17.25V21h3.75L17.81 9.94l-3.75-3.75z" fill="currentColor" v-if="workspace.visability === 0"/>
							<path d="M16.1 9l.9.9L7.9 19H7v-.9L16.1 9m3.6-6c-.2 0-.5.1-.7.3l-1.8 1.8 3.7 3.8L22.7 7c.4-.4.4-1 0-1.4l-2.3-2.3c-.2-.2-.5-.3-.7-.3m-3.6 3.2L5 17.2V21h3.8l11-11.1-3.7-3.7M8 5v-.5C8 3.1 6.9 2 5.5 2S3 3.1 3 4.5V5c-.6 0-1 .4-1 1v4c0 .6.4 1 1 1h5c.6 0 1-.4 1-1V6c0-.6-.4-1-1-1M7 5H4v-.5C4 3.7 4.7 3 5.5 3S7 3.7 7 4.5V5z" fill="currentColor" v-else-if="workspace.access === 'PROTECTED'"/>
							<path d="M12 17a2 2 0 01-2-2c0-1.11.89-2 2-2a2 2 0 012 2 2 2 0 01-2 2m6 3V10H6v10h12m0-12a2 2 0 012 2v10a2 2 0 01-2 2H6a2 2 0 01-2-2V10c0-1.11.89-2 2-2h1V6a5 5 0 015-5 5 5 0 015 5v2h1m-6-5a3 3 0 00-3 3v2h6V6a3 3 0 00-3-3z" fill="currentColor" v-else/>
						</svg>
					</div>
	...
	export default {
		...
		methods: {
			async getLaboratories() {
				this.laboratories = await Network.getLaboratoriesFromDb({
					laboratoryUuid: null,
					query: this.query,
			...
			async getWorkspaces() {
				this.workspaces = await Network.getWorkspacesFromDb({
					workspaceUuid: null,
					query: this.query,
					jwt: this.$store.state.jwt
				});
			...
			async getUserByJwt() {
				if (!this.$store.state.jwt) {
					this.$store.commit("setJwt", await Network.login({
						username: "demo",
						password: "demo"
					}));
				}
			...
			async updateSessionToken() {
				if (this.$store.state.jwt && this.userCurrent) {
					this.userGlobal = this.userCurrent = await Network.updateSessionToken({
						jwt: this.$store.state.jwt,
						uuid: this.userCurrent.uuid
					});
				} else {
					this.$router.replace({ path: '/login' });
				}
