	...
	<div>
		...
		<div class="row">
			<div class="col-3 mb-4">
				<div class="row mb-2">
					<div class="col-12">
						<button class="btn btn-block btn-outline-secondary mb-2" @click="order = !order; getChallenges()">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block mx-auto">
				...
				<div class="list-group">
					<template v-if="Object.keys(challenges).length !== 0">
						<div class="list-group-item list-group-item-action c-pointer" v-for="(challenge, uuid) in challenges" :key="uuid" @click.self="setViewMode(uuid)">
							<span class="d-block float-left mr-4" @click="setViewMode(uuid)">{{challenge.name}}</span>
							<div class="float-right">
								...
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block float-left" @click="deleteChallenge(uuid)">
			...
			<div class="col-9 mb-4">
							...
							<div class="card mb-4" v-for="scenario of challenge.scenarios">
								<div class="card-body">
									<div class="mr-auto">
										...
										<json-builder :template="scenario.input.value" :convert="true" :fixed="true" :fixed-values="true"/>
										<hr>
										<h6 class="mb-2">Output</h6>
										<json-builder :template="scenario.output.value" :convert="true" :fixed="true" :fixed-values="true"/>
									</div>
								</div>
							</div>
						</template>
				...
				<div class="card" v-if="mode === 'ADD' || mode === 'EDIT'">
					<div class="card-body">
							...
							<template v-if="challenge.hints.length !== 0">
								...
								<div class="list-group-item" v-for="(hint, index) in challenge.hints">
									<div class="row mx-0 flex-row">
										...
										<button class="btn btn-outline-secondary" @click="challenge.hints.splice(index, 1)">
						...
						<button class="btn btn-block btn-outline-secondary mb-4" @click="challenge.hints.push('')">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block mx-auto">
								<path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" fill="currentColor"/>
							</svg>
						</button>
								...
								<div class="card-body">
									<div class="mr-auto">
										<div class="float-right mt-2">
											<button class="btn btn-block btn-outline-secondary" @click="challenge.scenarios.splice(index, 1)">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block float-left">
													<path d="M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6v12M8 9h8v10H8V9m7.5-5l-1-1h-5l-1 1H5v2h14V4h-3.5z" fill="currentColor"/>
												</svg>
											</button>
										...
										<json-builder class="mb-4" :template="scenario.input.template" :convert="true" :fixed-root="true" @update:value="scenario.input.value = $event"/>

										<h5 class="mb-2">Output</h5>
										<json-builder :template="scenario.output.template" :convert="true" @update:value="scenario.output.value = $event"/>
									</div>
								</div>
							</div>
						</template>
						...
						<button class="btn btn-block btn-outline-secondary mt-2" @click="challenge.scenarios.push({'name':'New Scenario','input':{'value':{}, 'template':{}},'output':{'value':'', 'template':''}})">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block mx-auto">
								<path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" fill="currentColor"/>
							</svg>
						</button>
						...
						<button class="btn btn-block btn-outline-secondary" v-if="mode === 'ADD'" @click="addChallenge">Save Challenge</button>
						<button class="btn btn-block btn-outline-secondary" v-if="mode === 'EDIT'" @click="editChallenge">Save Challenge</button>
					</div>
				</div>
			</div>
		</div>
