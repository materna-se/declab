		...
		<div class="row">
			<div class="d-flex">
				<div class="mb-4 me-4" style="width: 25%; min-width: 25%;">
					<button class="btn btn-block btn-outline-primary mb-2" @click="order = !order; getOutputs()">
						<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="d-block mx-auto">
					...
					<div class="list-group">
						<template v-if="Object.keys(outputs).length !== 0">
							<div class="list-group-item list-group-item-action c-pointer d-flex align-items-center" v-for="(output, uuid) in outputs" :key="uuid" @click.self="setViewMode(uuid)">
								<span class="me-auto" @click="setViewMode(uuid)">
									<b>{{output.name}}</b>
									<template v-if="output.decision !== null">
										<br>
										<small class="text-muted text-break"><span class="badge bg-light text-dark">Decision</span> {{output.decision}}</small>
									</template>
								</span>
								...
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" @click="deleteOutput(uuid)">
				...
				<div class="flex-grow-1 mb-4">
					...
					<div class="card" v-if="mode === 'ADD' || mode === 'EDIT'">
						<div class="card-body">
							...
							<button class="btn btn-block btn-outline-primary" v-if="mode === 'ADD'" @click="addOutput">Save Output</button>
							<button class="btn btn-block btn-outline-primary" v-if="mode === 'EDIT'" @click="editOutput">Save Output</button>
						</div>
					</div>
				</div>
			</div>
