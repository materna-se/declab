					...
					<div class="list-group">
						<template v-if="Object.keys(tests).length !== 0">
							<div class="list-group-item list-group-item-action c-pointer d-flex align-items-center" style="display: flex; align-items: center" v-for="(test, uuid) in tests" :key="uuid" @click.self="setViewMode(uuid)" :class="[test.equal === undefined ? '' : (test.equal ? 'bg-success text-white' : 'bg-danger text-white')]">
								...
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" @click="deleteTest(uuid)">
						...
						<div class="card-body">
								...
								<div class="col-12">
									...
									<div class="card">
										<div class="card-body">
											...
											<json-builder :template="inputs[test.input].value" :convert="true" :fixed="true" :fixed-values="true"/>
										</div>
									</div>
								</div>
							</div>
							<div class="row">
								<div class="col-12">
									...
									<div class="card mt-2 border-lg" v-for="output in test.result.outputs" :key="test.uuid + '-' + output.decision" :class="[output.equal ? 'border-success' : 'border-danger']">
										<div class="card-body">
											<h5 class="mb-2">Name</h5>
					...
					<div class="card" v-if="mode === 'ADD' || mode === 'EDIT'">
						<div class="card-body">
							...
							<select id="form-output" size="15" multiple class="form-control mb-4" v-model="test.outputs">
								<option v-for="(output, uuid) in outputs" :key="uuid" :value="uuid">{{output.name}} {{output.decision === null ? null : ('(' + output.decision + ')')}}</option>
							</select>
